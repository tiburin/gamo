...
struct Forbid;
impl Forbid {
    fn start(mut store: Tipo, content: &str) -> Tipo {
...
impl App {
    ...
    fn forbid(&mut self, not_allow: String) -> &mut Self {
        let store: Tipo = HashSet::new();
        let mut content = format!("{}\n", not_allow);
        content.push_str(&self.off_content);
        self.store = Forbid::start(store, &content);
        self
    }
...
fn update_off_file(list: Vec<String>) {
    let mut acc = vec![];
    ...
    for word in Parse::lines(&fs::read_to_string("word.off").unwrap()) {
        if !store.contains(&word) {
            acc.push(word);
        }
    }
    acc.sort_by(|a, b| a.len().cmp(&b.len()));
    fs::write("word.off", acc.join("\n")).unwrap();
}
pub fn start(not_allow: Vec<String>) {
    ...
    let list = App::new(on_content, off_content)
        .forbid(not_allow.join("\n"))
        .start();
    Voc::new(list).direct_data().compose().write_to_files();
    update_off_file(not_allow);
}
