use crate::app::ReadParseFile;
use crate::app::Sphere;
...
impl File {
    ...
    fn setup(mut self, folder: &String, connector: &Connector) -> Self {
...
pub struct Release;
impl Release {
    pub fn write(connector: &Connector, sphere: &Sphere, report: &mut Vec<String>) {
        for folder in &sphere.config.types.list {
            for language in &connector.languages.folders {
                ...
                let new_content = File::parse_into_lines(content)
                    ...
                    .filter(|mas| {
                        let is_word = mas.word != "w" && mas.sentence == "s";
                        let is_sentence = mas.sentence != "s" && mas.word == "w";
                        let is_both = mas.word != "w" && mas.sentence != "s";
                        is_word || is_sentence || is_both
                    })
...
pub fn inner(connector: &Connector, sphere: &Sphere) {
    for folder in &sphere.config.types.list {
...
impl Languages {
    ...
    fn setup(mut self, connector: &Connector, sphere: &Sphere) -> Self {
